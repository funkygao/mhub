...
func (this *MemoryStore) Put(key string, message proto.Message) {
	...
	this.Unlock()
	log.Debug("memstore put -> key:%s, val:%#v", key, message)
}

func (this *MemoryStore) Get(key string) proto.Message {
	this.RLock()
	defer this.RUnlock()
	log.Debug("memstore get -> key:%s", key)
